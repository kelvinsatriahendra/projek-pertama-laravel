                ...
                <div class="swiper-slide">
                    <a href="{{ route('news.show', $banner->news->slug) }}" class="block">
                        <div class="relative flex flex-col gap-1 justify-end p-3 h-72 rounded-xl bg-cover bg-center overflow-hidden"
                            ...
                            <div class="absolute inset-x-0 bottom-0 h-full bg-gradient-to-t from-[rgba(0,0,0,0.4)] to-[rgba(0,0,0,0)]"></div>
        ...
        <div class="grid sm:grid-cols-1 gap-5 lg:grid-cols-4">
            @foreach ($featureds as $featured)
                <a href="{{ route('news.show', $featured->slug) }}">
                    <div class="relative border border-slate-200 p-3 rounded-xl hover:border-primary hover:cursor-pointer transition" style="height: 100%">
                        ...
                        <img src="{{ asset('storage/' . $featured->thumbnail) }}" alt="Featured News Thumbnail"
    ...
    <div class="flex flex-col px-4 md:px-10 lg:px-14 mt-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            @foreach ($news->skip(1) as $new)
                <a href="{{ route('news.show', $new->slug) }}" class="relative col-span-5 flex flex-col md:flex-row gap-3 border border-slate-200 p-3 rounded-xl hover:border-primary hover:cursor-pointer">
            ...
            @foreach ($authors as $author)
                ...
                <a href="{{ route('author.show', $author->username) }}">
                    <div class="flex flex-col items-center border border-slate-200 px-4 py-8 rounded-2xl hover:border-primary hover:cursor-pointer">
                        <img src="{{ asset('storage/' . $author->avatar) }}" alt="Author Avatar" class="rounded-full w-24 h-24">
            ...
            @foreach ($news as $choice)
                <a href="{{ route('news.show', $choice->slug) }}">
                    <div class="relative border border-slate-200 p-3 rounded-xl hover:border-primary hover:cursor-pointer transition" style="height: 100%;">
                        ...
                        <img src="{{ asset('storage/' . $choice->thumbnail) }}" alt="News Thumbnail"
